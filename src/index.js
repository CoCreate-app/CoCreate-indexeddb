      ...
      if (data.method == "database.read") {
        indexedDB.databases().then((databases) => {
          for (let database of databases) {
            ...
            } else newData.push({ $storage: ["indexeddb"], ...database });
          }
        ...
        for (let i = 0; i < databases.length; i++) {
          ...
          openRequest.onsuccess = function () {
            ...
            if (data.method == "database.update") {
              ...
              let version =
                dbVersion.get(database) || openRequest.result.version;
  ...
  if (data.method == "array.read") {
    ...
    await new Promise((resolve) => {
      request.onupgradeneeded = function (event) {
        ...
        let arrays;
        if (data.method == "array.update") arrays = Object.keys(data[type]);
          ...
          if (arrayExist && data.method == "array.create") {
            ...
            if (arrayExist) {
              if (data.method == "array.update") {
                ...
                else error = "An array with the new name already exist";
              }
...
async function processIndex(data, newData, database, array, type) {
  ...
  if (arrayExist) {
    ...
    if (data.method == "index.read") {
      ...
      db.close();
      let indexes;
      if (data.method == "index.update") indexes = Object.keys(data[type]);
      ...
      for (let i = 0; i < indexes.length; i++) {
        ...
        if (data.method == "index.create") {
          ...
          if (data.method == "index.delete") objectStore.deleteIndex(index);
          else if (
...
async function processObject(data, newData, database, array, type) {
  ...
  try {
    if (!data[type]) data[type] = [];
    else if (typeof data[type] === "string") data[type] = [{ _id: data[type] }];
    ...
    for (let i = 0; i < data[type].length; i++) {
      ...
      if (data.method == "object.create") {
        ...
        if (!data[type][i]._id) data[type][i]["_id"] = ObjectId().toString();
        ...
        if (data[type][i]._id) {
          ...
          if (data.$filter) {
            ...
            if (data.$filter.sort && data.$filter.sort.length > 0) {
              ...
              direction =
                data.$filter.sort[0].direction === "desc" ? "prev" : "next";
          ...
          (data.method == "object.update" && !range && !upsert && !isFilter)
        ) {
          continue;
        }
    ...
    request.onsuccess = async () => {
      ...
      if (
        ...
        if (isFilter) {
          ...
          else if (data.method === "object.update" && !upsert) isMatch = false;
        }
        ...
        if (index && !hasSkipped) {
          ...
          if (isMatch !== false) {
            ...
            if (data[type][i]._id) data[type][i] = { ...reference, ...result };
            else {
              let object = data[type].find(
                (obj) => obj._id && obj._id === result._id
              );
              if (object) object = { ...reference, ...object, ...result };
...
function filter(objectStore, data, object, result) {
  ...
  if (filter) {
    ...
    if (isMatch && filter.search) isMatch = searchData(result, filter.search);
  }
...
function createData(data, newData, type) {
  ...
  if (data.$filter) {
    if (data.$filter.sort) data[type] = sortData(data[type], data.$filter.sort);
